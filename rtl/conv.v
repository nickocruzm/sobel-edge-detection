`timescale 1ns / 1ps
/*

Inputs clock, reset, pixel_in

13 Ouputs

*/
module conv(
    input clk,
    input reset,
    input [7:0] pxl_in,
			
	output signed [15:0] reg_00, output signed [15:0] reg_01, output signed [15:0] reg_02, output signed [15:0] sr_0,
	output signed [15:0] reg_10, output signed [15:0] reg_11, output signed [15:0] reg_12, output signed [15:0] sr_1,
	output signed [15:0] reg_20, output signed [15:0] reg_21, output signed [15:0] reg_22,
	output signed [15:0] pxl_out,	
	output valid
);

// MODULE BODY

	//Define constants
	parameter N = 5;	//Image columns
	parameter M = 5;	//Image rows
	parameter K = 3; 	//Kernel size

	// Kernel weights — override per instance for Gx vs Gy
	parameter signed [7:0] K_00 = 1,  K_01 = 2,  K_02 = 1;
	parameter signed [7:0] K_10 = 0,  K_11 = 0,  K_12 = 0;
	parameter signed [7:0] K_20 = -1, K_21 = -2, K_22 = -1;

	initial begin
		$display("[%m] Kernel:");
		$display("  %0d  %0d  %0d", K_00, K_01, K_02);
		$display("  %0d  %0d  %0d", K_10, K_11, K_12);
		$display("  %0d  %0d  %0d", K_20, K_21, K_22);
	end

	// Intermediate wires
	wire signed [15:0] wire_00; wire signed [15:0] wire_01; wire signed [15:0] wire_02;
	wire signed [15:0] wire_10; wire signed [15:0] wire_11; wire signed [15:0] wire_12;
	wire signed [15:0] wire_20; wire signed [15:0] wire_21; wire signed [15:0] wire_22;

	// Row : 1
	mac mac_00(pxl_in, K_00, 0, wire_00);
	register r_00(clk, reset, wire_00, reg_00);

	mac mac_01(pxl_in, K_01, reg_00, wire_01);
	register r_01(clk, reset, wire_01, reg_01);

	mac mac_02(pxl_in, K_02, reg_01, wire_02);
	register r_02(clk, reset, wire_02, reg_02);

	shift #(.D(N-K)) row_1(clk, reg_02, sr_0);

	// Row : 2
	mac mac_10(pxl_in, K_10, sr_0, wire_10);
	register r_10(clk, reset, wire_10, reg_10);

	mac mac_11(pxl_in, K_11, reg_10, wire_11);
	register r_11(clk, reset, wire_11, reg_11);

	mac mac_12(pxl_in, K_12, reg_11, wire_12);
	register r_12(clk, reset, wire_12, reg_12);

	shift #(.D(N-K)) row_2(clk, reg_12, sr_1);

	// Row : 3
	mac mac_20(pxl_in, K_20, sr_1, wire_20);
	register r_20(clk, reset, wire_20, reg_20);

	mac mac_21(pxl_in, K_21, reg_20, wire_21);
	register r_21(clk, reset, wire_21, reg_21);

	mac mac_22(pxl_in, K_22, reg_21, wire_22);
	register r_22(clk, reset, wire_22, reg_22);

	assign pxl_out = reg_22;	

	// Valid bit logic

	reg [10:0] counter = 0;
	reg temp = 0;
  localparam LATENCY = 1;

	always @(posedge clk) begin
		counter <= counter + 1;

		// The logic below needs some revisiting to scale properly
		if (counter > ((K-1)*N + (K-1) + LATENCY) && counter < (M*N) + (K-1) + LATENCY) begin
			if ((counter - (K-1) - LATENCY ) % N > 1) begin
				temp <= 1;
				end
			else
				temp <= 0;
				end
		else
			temp <= 0; 
			end
					
	assign valid = temp;

	always @(posedge clk) begin
		if (valid)
			$display("[%m] pxl_in = %0d, pxl_out = %0d", pxl_in, pxl_out);
	end

endmodule